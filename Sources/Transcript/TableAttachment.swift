...
final class TableLayout {
  ...
  private let columns: [(x: CGFloat, width: CGFloat)]
  ...
  /// Water-fill the `budget` across columns: any column whose natural width already fits its fair
  /// share keeps it, the slack is split among the columns that don't, and no column is squeezed
  /// below its `floor` — its widest unbreakable fragment, so a squeezed column wraps between words
  /// (or CJK glyphs) instead of mid-word. So short columns stay their own width, wide multi-word
  /// columns wrap, and a lone long token like `transcript` still fits on one line. No-op when it
  /// all fits. If the floors themselves exceed the budget the table overflows the pane by that
  /// much — unavoidable, and rarer than the mid-word break it removes.
  private static func fit(_ natural: [CGFloat], floors: [CGFloat], budget: CGFloat) -> [CGFloat] {
    ...
    while !flexible.isEmpty {
      ...
      // A column that fits its fair share naturally is locked at natural (no wrap); one whose
      // floor already meets its share is locked at that floor (wraps, but no narrower). Both free
      // budget for the rest, so re-divide and look again.
      if let column = flexible.first(where: { natural[$0] <= fair }) {
        widths[column] = natural[column]
        remaining -= natural[column]
        flexible.remove(column)
      } else if let column = flexible.first(where: { floors[$0] >= fair }) {
        widths[column] = floors[column]
        remaining -= floors[column]
        flexible.remove(column)
      } else {
        for column in flexible { widths[column] = fair }
        break
      }
    }
    return widths
