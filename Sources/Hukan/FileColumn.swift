...
/// The source pane's text view: `WordSelectingTextView`'s reading of a double-click, and an undo
/// stack belonging to the file rather than to the window (see `UndoStackOwner`).
///
/// The stack is answered here rather than only through the window, so an edit registers against
/// the file whether or not the pane holds the focus at the time — a drop, a scripted `edit`, or a
/// rename arriving while the panel is focused all reach the buffer, and each of them belongs to
/// the file's own ⌘Z.
final class EditorTextView: WordSelectingTextView, UndoStackOwner {
