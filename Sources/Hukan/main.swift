...
final class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  private static func makeMainMenu() -> NSMenu {
    ...
    fileMenu.addItem(
      ...
      keyEquivalent: "s")
    fileMenu.addItem(.separator())
    // ⌘W is deliberately left unbound, reserved for Close Tab once the right column carries
    // web/file tabs — the browser split where ⌘W closes a tab and ⌘⇧W the window. Close
    // Window escalates now to ⌘⇧W (uppercase "W") to leave that room. Close Repository, the
    // deliberate open/close unit, carries no key equivalent: it is a rail action (right-click
    // a repository) the way Zed removes a folder from its project panel, kept in this menu
    ...
    // rewrites each to name the action it would perform ("Show Source" ⇄ "Show Diff"), so the
    // menu reads the current state instead of a both-ways slash.
    let viewItem = NSMenuItem()
    let viewMenu = NSMenu(title: "View")
    viewMenu.addItem(
      withTitle: "Overview", action: #selector(WorkspaceWindowController.toggleOverview(_:)),
      keyEquivalent: "0")
    viewMenu.addItem(
      withTitle: "Show Source", action: #selector(WorkspaceWindowController.toggleDiffMode(_:)),
      keyEquivalent: "d")
    viewMenu.addItem(
      withTitle: "Show All Files",
      action: #selector(WorkspaceWindowController.toggleFileSidebarMode(_:)), keyEquivalent: "\\")
